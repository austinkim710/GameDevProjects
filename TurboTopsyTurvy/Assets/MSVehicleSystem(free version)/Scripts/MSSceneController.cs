...
public class MSSceneController : MonoBehaviour {
	...
	string velocitymph;
	string handBrake;
	MSVehicleController vehicleCode;
				...
				if (vehicles.Length <= 1) {
					...
					for (int x = 0; x < vehicles.Length; x++) {
						...
						for (int y = 0; y < controllerTemp.doorPosition.Length; y++) {
							...
							proximityDistanceTemp = Vector3.Distance (player.transform.position, vehicles [proximityObjectIndex].GetComponent<MSVehicleController> ().doorPosition[proximityDoorIndex].transform.position);
	...
	void EnableVehicle(int index){
		...
		if (index != currentVehicle) {
			if (vehicles [currentVehicle]) {
				for (int x = 0; x < vehicles.Length; x++) {
					vehicles [x].GetComponent<MSVehicleController> ().ExitTheVehicle ();
				}
				vehicles [currentVehicle].GetComponent<MSVehicleController> ().EnterInVehicle ();
			}
		}
