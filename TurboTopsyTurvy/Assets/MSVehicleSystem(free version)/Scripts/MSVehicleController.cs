...
public class CameraTypeClass {
	...
	public enum TipoRotac{FollowPlayer, Orbital, StraightStop,ETS_StyleCamera}
...
public class MSVehicleController : MonoBehaviour {
	...
	void SetCameras(){
		...
		for (int x = 0; x < _cameras.cameras.Length; x++) {
			...
			if (!_cameras.cameras [x]._camera) {
				...
				_cameras.cameras [x].rotationType = CameraTypeClass.TipoRotac.Orbital;
			}
			if (_cameras.cameras [x].rotationType == CameraTypeClass.TipoRotac.StraightStop) {
			...
			if (_cameras.cameras [x].rotationType == CameraTypeClass.TipoRotac.FollowPlayer) {
				startPositionCameras [x] = new GameObject ("positionCameraFollow"+x);
				startPositionCameras [x].transform.parent = transform;
				startPositionCameras [x].transform.localPosition = _cameras.cameras [x]._camera.transform.localPosition;
				if (_cameras.setLayers) {
					AjustLayers ();
				}
			}
			if (_cameras.cameras [x].rotationType == CameraTypeClass.TipoRotac.Orbital) {
	...
	void CamerasManager(){
		...
		case CameraTypeClass.TipoRotac.StraightStop:
			...
			break;
		case CameraTypeClass.TipoRotac.FollowPlayer:
			if (!Physics.Linecast (transform.position, startPositionCameras [indexCamera].transform.position)) {
				_cameras.cameras [indexCamera]._camera.transform.position = Vector3.Lerp (_cameras.cameras [indexCamera]._camera.transform.position, startPositionCameras [indexCamera].transform.position, Time.deltaTime * 5.0f);
			}
			else if (Physics.Linecast (transform.position, startPositionCameras [indexCamera].transform.position, out hitCameras)) {
				_cameras.cameras [indexCamera]._camera.transform.position = Vector3.Lerp (_cameras.cameras [indexCamera]._camera.transform.position, hitCameras.point, Time.deltaTime * 5.0f);
			}
			_cameras.cameras [indexCamera]._camera.transform.LookAt (transform);
			break;
		case CameraTypeClass.TipoRotac.Orbital:
			minDistanceOrbitalCamera = 5.0f;
			if (!Physics.Linecast (transform.position, _cameras.cameras [indexCamera]._camera.transform.position)) {
				...
				minDistanceOrbitalCamera = Mathf.Clamp ((Vector3.Distance (transform.position, hitCameras.point)), (2.5f), 500);
			}
			//
			xOrbit [indexCamera] += mouseXInput * (0.8f * distanceOrbitCamera [indexCamera])/(distanceOrbitCamera [indexCamera]*0.5f);
			yOrbit [indexCamera] -= mouseYInput * 4;
			...
			distanceOrbitCamera [indexCamera] = Mathf.Clamp (distanceOrbitCamera [indexCamera] - (mouseScrollWheelInput * 50.0f), minDistanceOrbitalCamera, 500);
